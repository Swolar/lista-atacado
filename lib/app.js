...
const PARTNERS = [
  ...
  {
    slug: 'bross',
    name: process.env.P2_NAME || 'Bross',
    login: (process.env.P2_USER || 'bross').trim().toLowerCase(),
    password: process.env.P2_PASSWORD || 'Bross#',
...
function ensureReady() {
  if (!readyPromise) {
    readyPromise = (async () => {
      try {
        // a coluna pass_hash é a adição mais recente ao schema: se ela (ou a própria
        // tabela) faltar, roda o DDL idempotente — cobre banco novo E banco antigo
        await q(`select pass_hash from ${T('partner_data')} limit 1`);
      } catch (err) {
...
// ---------- senha própria dos sócios ----------
// Cada sócio cria a própria senha por um link de primeiro acesso (/senha/<slug>/<token>).
// A senha fica com hash scrypt em partner_data; enquanto não existir, vale a senha das
// variáveis de ambiente (P1_PASSWORD/P2_PASSWORD ou o padrão embutido).
...
// hash salvo do sócio (ou null se ele ainda não criou senha própria)
async function partnerCreds(slug) {
  const r = await q(`select pass_salt, pass_hash from ${T('partner_data')} where slug = $1`, [slug]);
  const row = r.rows[0];
  return row && row.pass_hash ? { salt: row.pass_salt, hash: row.pass_hash } : null;
}

// token do link de primeiro acesso — derivado por HMAC da DATABASE_URL (nenhum
// provisionamento no banco). O link só funciona enquanto o sócio não tem senha própria;
// para reemitir (sócio esqueceu a senha), zere pass_hash na tabela partner_data.
function setupToken(slug) {
  return crypto
    .createHmac('sha256', String(process.env.DATABASE_URL || 'local'))
    .update('senha-setup:' + slug)
    .digest('hex')
    .slice(0, 32);
}

// fingerprint gravada na sessão: trocar a senha (no banco ou no ambiente) derruba
// na hora as sessões antigas daquele sócio
function partnerFingerprint(partner, creds) {
  return passFingerprint(creds ? creds.hash : partner.password);
}
...
async function partnerOf(req, url) {
  ...
  if (row.fingerprint && row.fingerprint !== partnerFingerprint(partner, await partnerCreds(row.subject))) {
...
async function handleApi(req, res, url) {
  ...
  // ---- primeiro acesso do sócio: criar a própria senha via link ----
  if (method === 'GET' && p === '/api/admin/setup-info') {
    const slug = String(url.searchParams.get('socio') || '');
    const token = String(url.searchParams.get('token') || '');
    const partner = partnerBySlug(slug);
    if (!partner || !timingEqual(token, setupToken(slug))) {
      return json(res, 404, { error: 'Link inválido.' });
    }
    const creds = await partnerCreds(slug);
    return json(res, 200, { name: partner.name, login: partner.login, alreadySet: !!creds });
  }

  if (method === 'POST' && p === '/api/admin/setup-password') {
    const ip = ipOf(req);
    if (await throttled('adminsetup', ip)) {
      return json(res, 429, { error: 'Muitas tentativas. Aguarde um minuto e tente de novo.' });
    }
    const body = await readBody(req);
    const slug = String(body.socio || '');
    const token = String(body.token || '');
    const partner = partnerBySlug(slug);
    if (!partner || !timingEqual(token, setupToken(slug))) {
      await throttleHit('adminsetup', ip);
      return json(res, 404, { error: 'Link inválido.' });
    }
    if (await partnerCreds(slug)) {
      return json(res, 409, { error: 'A senha deste painel já foi criada. Se precisar trocar, fale com quem administra o sistema.' });
    }
    const password = String(body.password || '');
    if (password.length < 6) return json(res, 400, { error: 'A senha precisa ter pelo menos 6 caracteres.' });
    const { salt, hash } = hashPassword(password);
    await q(
      `insert into ${T('partner_data')} (slug, pass_salt, pass_hash) values ($1, $2, $3)
       on conflict (slug) do update set pass_salt = excluded.pass_salt, pass_hash = excluded.pass_hash`,
      [slug, salt, hash]
    );
    // qualquer sessão antiga daquele sócio cai (fingerprint mudou)
    await q(`delete from ${T('sessions')} where kind = 'admin' and subject = $1`, [slug]).catch(() => {});
    return json(res, 200, { ok: true, login: partner.login });
  }
  ...
  if (method === 'POST' && p === '/api/admin/login') {
    ...
    // cada sócio tem usuário + senha próprios; a senha criada pelo sócio (banco)
    // tem precedência sobre a das variáveis de ambiente
    const partner = PARTNERS.find((pt) => pt.login === username) || null;
    const creds = partner ? await partnerCreds(partner.slug) : null;
    const ok =
      partner &&
      (creds ? timingEqual(hashPassword(pass, creds.salt).hash, creds.hash) : timingEqual(pass, partner.password));
    if (!ok) {
      await throttleHit('adminlogin', ip);
      return json(res, 401, { error: 'Usuário ou senha incorretos.' });
    }
    await throttleClear('adminlogin', ip);
    const token = await createSession('admin', partner.slug, partnerFingerprint(partner, creds));
...
function logConfigWarnings(log = console.log) {
  const comPadrao = PARTNERS.filter((pt) => pt.password === 'Krauz#' || pt.password === 'Bross#').map((x) => x.slug);
  if (comPadrao.length) {
    log(
      `⚠ Senha de ambiente PADRÃO em: ${comPadrao.join(', ')} — cada sócio deve criar a própria senha pelo link de primeiro acesso (node scripts/print-setup-links.js) ou defina P1_PASSWORD/P2_PASSWORD.`
    );
  }
...
module.exports = { handleRequest, handleApi, PARTNERS, logConfigWarnings, ensureReady, setupToken };
