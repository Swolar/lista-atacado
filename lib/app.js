...
async function createSession(kind, subject) {
  const token = newId(16);
  await q(
    `insert into ${T('sessions')} (token, kind, subject, expires_at) values ($1, $2, $3, now() + interval '${SESSION_DAYS} days')`,
    [token, kind, subject]
  );
  if (Math.random() < 0.05) {
...
async function partnerOf(req, url) {
  ...
  const r = await q(
    `select subject from ${T('sessions')} where token = $1 and kind = 'admin' and expires_at > now()`,
    [token]
  );
  const slug = r.rows[0] && r.rows[0].subject;
  return slug && PARTNER_SLUGS.has(slug) ? slug : null;
}
...
async function handleApi(req, res, url) {
      ...
      async (client) => {
        ...
        for (let attempt = 0; attempt < 8; attempt++) {
          ...
          try {
            ...
            if (err.code === UNIQUE_VIOLATION && attempt < 7) continue;
            throw err;
          }
        }
  ...
  if (method === 'POST' && p === '/api/admin/login') {
    ...
    const token = await createSession('admin', partner.slug);
      ...
      async (client) => {
        ...
        if (toUpdate.length) {
          await client.query(
            ...
            [toUpdate.map((x) => x.id), toUpdate.map((x) => x.stock)]
          );
        }
        if (toCreate.length) {
          await client.query(
...
async function handleRequest(req, res, url) {
  try {
    ...
    if (status === 500) console.error('Erro inesperado na API:', err);
    if (!res.headersSent) {
      json(res, status, { error: err instanceof ApiError ? err.message : 'Erro interno. Tente de novo.' });
    }
  }
}
